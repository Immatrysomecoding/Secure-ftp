...
// Kernel 1: Basic version using only global memory
__global__ void blurImgKernel1(uchar3 * inPixels, int width, int height, 
        float * filter, int filterWidth, 
        uchar3 * outPixels)
{
	int outR = blockIdx.y * blockDim.y + threadIdx.y;
	int outC = blockIdx.x * blockDim.x + threadIdx.x;
	
	if (outR < height && outC < width)
	{
		...
		for (int filterR = 0; filterR < filterWidth; filterR++)
		{
			for (int filterC = 0; filterC < filterWidth; filterC++)
			{
				...
				int inR = outR - filterRadius + filterR;
				int inC = outC - filterRadius + filterC;
		...
		outPixels[outR * width + outC] = make_uchar3(outPixel.x, outPixel.y, outPixel.z);
	}
}

// Kernel 2: Using shared memory for input pixels
        ...
        uchar3 * outPixels)
{
	...
	// Calculate shared memory dimensions (block + halo)
	int s_width = blockDim.x + 2 * filterRadius;
	int s_height = blockDim.y + 2 * filterRadius;
	
	// Load data from global memory to shared memory
	// Each thread loads multiple elements if needed
	for (int i = threadIdx.y; i < s_height; i += blockDim.y)
	{
		for (int j = threadIdx.x; j < s_width; j += blockDim.x)
		{
			int inR = blockIdx.y * blockDim.y + i - filterRadius;
			int inC = blockIdx.x * blockDim.x + j - filterRadius;
			
			// Clamp boundary
			inR = min(max(0, inR), height - 1);
			inC = min(max(0, inC), width - 1);
			
			s_inPixels[i * s_width + j] = inPixels[inR * width + inC];
		}
	}
	...
	if (outR < height && outC < width)
	{
		...
		for (int filterR = 0; filterR < filterWidth; filterR++)
		{
			for (int filterC = 0; filterC < filterWidth; filterC++)
			{
				...
				int s_r = threadIdx.y + filterR;
				int s_c = threadIdx.x + filterC;
				
				uchar3 inPixel = s_inPixels[s_r * s_width + s_c];
...
// Kernel 3: Using shared memory for input pixels and constant memory for filter
        ...
        uchar3 * outPixels)
{
	...
	// Compute output pixel using constant memory filter
	if (outR < height && outC < width)
	{
		float3 outPixel = make_float3(0, 0, 0);
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	{
		// Host implementation
		printf("Blurring image on host...\n");
		int filterRadius = filterWidth / 2;
		
		for (int outR = 0; outR < height; outR++)
		{
			for (int outC = 0; outC < width; outC++)
			{
				float3 outPixel = make_float3(0, 0, 0);
	...
	else // Use device
	{
		...
		{
			int filterRadius = filterWidth / 2;
			int s_width = blockSize.x + 2 * filterRadius;
			int s_height = blockSize.y + 2 * filterRadius;
			size_t smemSize = s_width * s_height * sizeof(uchar3);
